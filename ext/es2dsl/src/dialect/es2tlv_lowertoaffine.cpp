...
using AddOpLowering = BinaryOpLowering<tolva::AddOp, AddFOp>;
using MulOpLowering = BinaryOpLowering<tolva::MulOp, MulFOp>;

//===----------------------------------------------------------------------===//
// ToyToAffine RewritePatterns: Constant operations
//===----------------------------------------------------------------------===//

struct ConstantOpLowering : public OpRewritePattern<tolva::ConstantOp> {
  using OpRewritePattern<tolva::ConstantOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(tolva::ConstantOp op, PatternRewriter& rewriter) const final {
    ...
    std::function<void(uint64_t)> storeElements = [&](uint64_t dimension) {
      ...
      if (dimension == valueShape.size()) {
        rewriter.create<AffineStoreOp>(loc, rewriter.create<ConstantOp>(loc, *valueIt++), alloc, llvm::makeArrayRef(indices));
        return;
      }
...
struct ReturnOpLowering : public OpRewritePattern<tolva::ReturnOp> {
  using OpRewritePattern<tolva::ReturnOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(tolva::ReturnOp op, PatternRewriter& rewriter) const final {
...
struct TransposeOpLowering : public ConversionPattern {
  ...
  LogicalResult matchAndRewrite(Operation* op, ArrayRef<Value> operands, ConversionPatternRewriter& rewriter) const final {
    ...
    lowerOpToLoops(op, operands, rewriter, [loc](OpBuilder& builder, ValueRange memRefOperands, ValueRange loopIvs) {
      ...
      tolva::TransposeOpAdaptor transposeAdaptor(memRefOperands);
...
namespace {
struct ToyToAffineLoweringPass : public PassWrapper<ToyToAffineLoweringPass, FunctionPass> {
  void getDependentDialects(DialectRegistry& registry) const override { registry.insert<AffineDialect, StandardOpsDialect>(); }
...
void ToyToAffineLoweringPass::runOnFunction() {
  ...
  target.addIllegalDialect<tolva::TolvaDialect>();
  target.addLegalOp<tolva::PrintOp>();

  // Now that the conversion target has been defined, we just need to provide
  // the set of patterns that will lower the Toy operations.
  OwningRewritePatternList patterns;
  patterns.insert<AddOpLowering, ConstantOpLowering, MulOpLowering, ReturnOpLowering, TransposeOpLowering>(&getContext());

  // With the target and rewrite patterns defined, we can now attempt the
  // conversion. The conversion will signal failure if any of our `illegal`
  // operations were not converted successfully.
  if (failed(applyPartialConversion(getFunction(), target, patterns))) signalPassFailure();
}

/// Create a pass for lowering operations in the `Affine` and `Std` dialects,
/// for a subset of the Toy IR (e.g. matmul).
std::unique_ptr<Pass> mlir::tolva::createLowerToAffinePass() {
