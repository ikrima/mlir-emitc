...
inline LogicalResult interleaveWithError(ForwardIterator begin, ForwardIterator end, UnaryFunctor each_fn, NullaryFunctor between_fn) {
  if (begin == end) return success();
  if (failed(each_fn(*begin))) return failure();
  ++begin;
  for (; begin != end; ++begin) {
    between_fn();
    if (failed(each_fn(*begin))) return failure();
...
inline LogicalResult interleaveWithError(const Container& c, UnaryFunctor each_fn, NullaryFunctor between_fn) {
  return interleaveWithError(c.begin(), c.end(), each_fn, between_fn);
}

template <typename Container, typename UnaryFunctor>
inline LogicalResult interleaveCommaWithError(const Container& c, raw_ostream& os, UnaryFunctor each_fn) {
...
static LogicalResult printConstantOp(CppEmitter& emitter, tolva::ConstantOp constantOp) {
  auto& os = emitter.ostream();
  emitter.emitType(constantOp.getType());
  os << " " << emitter.getOrCreateName(constantOp.getResult());
  if (failed(emitter.emitAttribute(constantOp.value()))) return constantOp.emitError("unable to emit constant value");
  return success();
}

static LogicalResult printCallOp(CppEmitter& emitter, CallOp callOp) {
  if (failed(emitter.emitAssignPrefix(*callOp.getOperation()))) return failure();

  auto& os = emitter.ostream();
  os << callOp.getCallee() << "(";
  if (failed(emitter.emitOperands(*callOp.getOperation()))) return failure();
  os << ")";
  return success();
}
...
static LogicalResult printCallOp(CppEmitter& emitter, emitc::CallOp callOp) {
  ...
  if (failed(emitter.emitAssignPrefix(op))) return failure();
  os << callOp.callee();
  ...
  auto emitArgs = [&](Attribute attr) -> LogicalResult {
    if (auto t = attr.dyn_cast<IntegerAttr>()) {
      ...
      if (t.getType().isIndex()) {
        auto idx = t.getInt();
        if ((idx < 0) || (idx >= op.getNumOperands())) return op.emitOpError() << "invalid operand index";
        if (!emitter.hasValueInScope(op.getOperand(idx))) return op.emitOpError() << "operand " << idx << "'s value not defined in scope";
  ...
  if (callOp.template_args()) {
    os << "<";
    if (failed(interleaveCommaWithError(*callOp.template_args(), os, emitArgs))) return failure();
    os << ">";
  }
  ...
  //  callOp.dump();
  //}
  auto emittedArgs = callOp.args() ? interleaveCommaWithError(*callOp.args(), os, emitArgs) : emitter.emitOperands(op);
  if (failed(emittedArgs)) return failure();
  os << ")";
  return success();
}
...
static LogicalResult printForOp(CppEmitter& emitter, emitc::ForOp forOp) {
  ...
  os << "for (";
  if (failed(emitter.emitType(forOp.getInductionVar().getType()))) return failure();
  os << " ";
...
static LogicalResult printIfOp(CppEmitter& emitter, emitc::IfOp ifOp) {
  ...
  os << "if (";
  if (failed(emitter.emitOperands(*ifOp.getOperation()))) return failure();
  os << ") {\n";
...
static LogicalResult printYieldOp(CppEmitter& emitter, emitc::YieldOp yieldOp) {
  ...
  if (yieldOp.getNumOperands() == 0) {
    ...
    for (uint32_t result = 0; result < parentOp.getNumResults(); ++result) {
      os << emitter.getOrCreateName(parentOp.getResult(result)) << " = ";

      if (!emitter.hasValueInScope(yieldOp.getOperand(result))) return yieldOp.emitError() << "operand value not in scope";
...
static LogicalResult printReturnOp(CppEmitter& emitter, tolva::ReturnOp returnOp) {
  auto& os = emitter.ostream();
  os << "return";
  switch (returnOp.getNumOperands()) {
  case 0: return success();
  case 1: os << " " << emitter.getOrCreateName(returnOp.getOperand(0)); return success(emitter.hasValueInScope(returnOp.getOperand(0)));
  default:
    os << " std::make_tuple(";
    if (failed(emitter.emitOperandsAndAttributes(*returnOp.getOperation()))) return failure();
    os << ")";
    return success();
  }
}
...
static LogicalResult printModule(CppEmitter& emitter, ModuleOp moduleOp) {
  ...
  for (FuncOp funcOp : moduleOp.getOps<FuncOp>()) {
    ...
    os << " " << funcOp.getName() << "(";
    if (failed(interleaveCommaWithError(funcOp.getArguments(), os, [&](BlockArgument arg) { return emitter.emitType(arg.getType()); })))
      return failure();
    os << ");\n";
  }
  os << "\n";
  ...
  for (Operation& op : moduleOp) {
    if (failed(emitter.emitOperation(op, /*trailingSemiColon=*/false))) return failure();
  }
  return success();
}
...
static LogicalResult printFunction(CppEmitter& emitter, FuncOp functionOp) {
  ...
  if (blocks.size() != 1) return functionOp.emitOpError() << "only single block functions supported";

  CppEmitter::Scope scope(emitter);
  auto&             os = emitter.ostream();
  if (failed(emitter.emitTypes(functionOp.getType().getResults()))) return functionOp.emitError() << "unable to emit all types";
  os << " " << functionOp.getName();

  os << "(";
  if (failed(interleaveCommaWithError(functionOp.getArguments(), os, [&](BlockArgument arg) -> LogicalResult {
        if (failed(emitter.emitType(arg.getType())))
          return functionOp.emitError() << "unable to emit arg " << arg.getArgNumber() << "'s type";
...
CppEmitter::CppEmitter(raw_ostream& os)
  : os(os) {
...
StringRef CppEmitter::getOrCreateName(Value val) {
  if (!mapper.count(val)) mapper.insert(val, formatv("v{0}", ++valueInScopeCount.top()));
  return *mapper.begin(val);
}

bool CppEmitter::mapToSigned(IntegerType::SignednessSemantics val) {
  switch (val) {
  case IntegerType::Signless: return true;
...
LogicalResult CppEmitter::emitAttribute(Attribute attr) {
  ...
  auto printFloat = [&](APFloat val) {
    if (val.isFinite()) {
      ...
      case llvm::APFloatBase::S_IEEEsingle: os << "(float)"; break;
      case llvm::APFloatBase::S_IEEEdouble: os << "(double)"; break;
      default: break;
      };
      os << strValue;
    }
  ...
  if (auto dense = attr.dyn_cast<DenseIntElementsAttr>()) {
    if (auto iType = dense.getType().cast<TensorType>().getElementType().cast<IntegerType>()) {
      os << '{';
      interleaveComma(dense, os, [&](APInt val) { printInt(val, mapToSigned(iType.getSignedness())); });
      os << '}';
      return success();
    }
    if (auto iType = dense.getType().cast<TensorType>().getElementType().cast<IndexType>()) {
      os << '{';
...
LogicalResult CppEmitter::emitOperands(Operation& op) {
  auto emitOperandName = [&](Value result) -> LogicalResult {
    if (!hasValueInScope(result)) return op.emitError() << "operand value not in scope";
...
LogicalResult CppEmitter::emitOperandsAndAttributes(Operation& op, ArrayRef<StringRef> exclude) {
  if (failed(emitOperands(op))) return failure();
  ...
  auto emitNamedAttribute = [&](NamedAttribute attr) -> LogicalResult {
    if (llvm::is_contained(exclude, attr.first.strref())) return success();
    os << "/* " << attr.first << " */";
    if (failed(emitAttribute(attr.second))) return op.emitError() << "unable to emit attribute " << attr.second;
    return success();
  };
...
LogicalResult CppEmitter::emitAssignPrefix(Operation& op) {
  ...
  case 1: {
    ...
    if (failed(emitType(result.getType()))) return op.emitError() << "unable to emit type " << result.getType();
    os << " " << getOrCreateName(result) << " = ";
    break;
  }
  default:
    for (auto result : op.getResults()) {
      if (failed(emitType(result.getType()))) return failure();
      os << " " << getOrCreateName(result) << ";\n";
    }
    os << "std::tie(";
...
static LogicalResult printOperation(CppEmitter& emitter, Operation& op) {
  ...
  // if (auto callOp     = dyn_cast<emitc::CallOp>(op))  { return printCallOp(emitter, callOp);         }
  // if (auto ifOp       = dyn_cast<emitc::IfOp>(op))    { return printIfOp(emitter, ifOp);             }
  // if (auto yieldOp    = dyn_cast<emitc::YieldOp>(op)) { return printYieldOp(emitter, yieldOp);       }
  // if (auto forOp      = dyn_cast<emitc::ForOp>(op))   { return printForOp(emitter, forOp);           }
  // if (auto constantOp = dyn_cast<ConstantOp>(op))     { return printConstantOp(emitter, constantOp); }
  // if (auto returnOp   = dyn_cast<ReturnOp>(op))       { return printReturnOp(emitter, returnOp);     }
...
LogicalResult CppEmitter::emitType(Type type) {
  ...
  else if (auto itype = type.dyn_cast<FloatType>()) {
    switch (itype.getWidth()) {
    case 32: return (os << "float"), success();
    case 64: return (os << "double"), success();
    default: break;
    }
  }
  ...
  else if (auto itype = type.dyn_cast<TensorType>()) {
    if (itype.hasRank()) {
      os << "Tensor";
      os << itype.getRank();
      os << "D<";
  ...
  else if (auto ot = type.dyn_cast<OpaqueType>()) {
    os << ot.getTypeData();
    return success();
  }
  type.dump();
...
LogicalResult CppEmitter::emitTypes(ArrayRef<Type> types) {
  ...
  case 1: return emitType(types.front());
  default: return emitTupleType(types);
  }
}

LogicalResult CppEmitter::emitTupleType(ArrayRef<Type> types) {
  os << "std::tuple<";
  if (failed(interleaveCommaWithError(types, os, [&](Type type) { return emitType(type); }))) return failure();
  os << ">";
  return success();
}

LogicalResult mlir::tolva::TranslateToCpp(Operation& op, raw_ostream& os, bool trailingSemicolon) {
  return CppEmitter(os).emitOperation(op, trailingSemicolon);
}
